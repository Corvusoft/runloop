...
SCENARIO( "Launching tasks after a specific delay" )
{
    GIVEN( "I have setup a new runloop" )
    {
        ...
        WHEN( "I launch a task at after a specific delay" )
        {
            runloop->launch_in( delay, [ &task_called, datestamp, runloop ]( void )
            {
                runloop->stop( );
                task_called = true;
